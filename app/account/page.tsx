...
export default function AccountPage() {
  ...
  return <main className="route account-route"><SiteHeader active="account" /><section className="account-stage">
    <div className="account-brand"><p className="micro-label light"><span /> YOUR DROPWICH ACCOUNT</p><h1>Order faster.<br /><em>Stay connected.</em></h1><p>Sign in before placing an order. Administrator accounts also unlock the private operations and finance dashboard.</p><div><span><Check /> Saved orders</span><span><Check /> Protected admin access</span><span><Check /> Secure password hashing</span></div></div>
    <div className="account-card">
      {loading ? <p>Loading your account…</p> : user ? <div className="signed-in"><span><UserRound /></span><small>SIGNED IN AS</small><h2>{user.name}</h2><p>@{user.username} · {user.email}</p><b className={`role-badge ${user.role}`}>{user.role === "admin" ? <ShieldCheck /> : <UserRound />}{user.role}</b>{user.role === "admin" && <Link href="/dashboard">Open admin dashboard <ArrowRight /></Link>}<button onClick={logout}><LogOut /> Sign out</button></div> : <><div className="auth-tabs"><button className={mode === "signin" ? "active" : ""} onClick={() => { setMode("signin"); setError(""); }}>Sign in</button><button className={mode === "signup" ? "active" : ""} onClick={() => { setMode("signup"); setError(""); }}>Create account</button></div><form onSubmit={submit}>{mode === "signup" && <><label>Full name<input name="name" autoComplete="name" required minLength={2} /></label><label>Email address<input name="email" type="email" autoComplete="email" required /></label><label>Username<input name="username" autoComplete="username" required minLength={3} pattern="[a-zA-Z0-9._-]+" /></label></>}{mode === "signin" && <label>Username or email<input name="login" autoComplete="username" required /></label>}<label>Password<input name="password" type="password" autoComplete={mode === "signin" ? "current-password" : "new-password"} required minLength={10} /></label>{mode === "signup" && <small>Use at least 10 characters with uppercase, lowercase, and a number.</small>}{error && <p className="form-error" role="alert">{error}</p>}<button type="submit">{mode === "signin" ? "Sign in" : "Create account"} <ArrowRight /></button></form><Link className="admin-setup-link" href="/admin/setup">First-time administrator setup</Link></>}
    </div>
  </section><div className="route-footer dark-footer"><span>SECURE ACCOUNT ACCESS</span><p>Passwords are hashed and sessions use protected cookies.</p><span>ACCOUNT</span></div></main>;
}
