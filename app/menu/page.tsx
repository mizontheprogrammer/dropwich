...
export default function MenuPage() {
  ...
  const openCustomizer = (id: ProductId) => {
    ...
    requestAnimationFrame(() =>
      document.getElementById("customize-order")?.scrollIntoView({ behavior: "smooth", block: "start" }),
    );
  };
  ...
  return (
    <main className={`route menu-route tone-${product.tone}`}>
      <SiteHeader active="menu" />

      <section className="menu-stage">
        <div className="menu-index">
          <p className="micro-label"><span /> ORIGINAL 2023 LINEUP</p>
          <div className="menu-tabs" role="tablist" aria-label="Dropwich menu">
            {products.map(item => (
              <button
                key={item.id}
                role="tab"
      ...
      <section className="menu-gallery">
        <header><p className="micro-label"><span /> MADE YOUR WAY</p><h2>Three originals.<br />Plenty of combinations.</h2></header>
        <div>
          {products.map(item => (
            <button type="button" className="menu-gallery-card" key={item.id} onClick={() => openCustomizer(item.id)} aria-label={`Customize ${item.label}`}>
              <span className="gallery-display-name">{item.label.split(" ").map(word => <span key={word}>{word}</span>)}</span>
              <figure>
                <Image className="gallery-image gallery-image-rest" src={item.image} unoptimized alt="" width={1254} height={1254} />
                <Image className="gallery-image gallery-image-reveal" src={item.image} unoptimized alt={`${item.label} in Dropwich packaging`} width={1254} height={1254} />
              </figure>
        ...
        <div className="order-overlay" onMouseDown={() => setDrawer(false)}>
          <aside onMouseDown={event => event.stopPropagation()} aria-labelledby="order-title" role="dialog" aria-modal="true">
            ...
            {status === "complete" ? (
              ...
              <div className="order-message"><ShoppingBag /><h3>Sign in to order.</h3><p>Create an account or sign in, then return to place your order.</p><Link className="order-link" href="/account?returnTo=/menu">Continue to sign in <ChevronRight /></Link></div>
            ) : cart.length ? (
              <>
